...
function alerta($postdata, $primary, $xcrud){
//    $postdata->set('password', sha1( $postdata->get('password') ));
echo '<script>
   $.get("http://localhost/ingestec/ingenieros/Correo", {id_correo: "1", id: "Nw==", m: "mail"}, function(respuesta){
      $("#miparrafo").html(respuesta);
   });
  </script>';
}

function facturas_detalle_insert($postdata, $xcrud)
{
    $postdata->set('monto_desglose', ( $postdata->get('cantidad') * $postdata->get('precio_unitarios') ));
}
