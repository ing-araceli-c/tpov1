<?php class Sys_Detalle6 extends CI_Controller {
	...
	return $valor;
	 ...
	 $this->load->view('system/base/Menu', $data, FALSE);
         ...
         if (isset($_GET['so'])) {
            ...
            $this->Tpo_model->initialize("tab_sujetos_obligados");
            $sujeto_obligado = $this->Tpo_model->find1("id_sujeto_obligado", $string, 1);
            setD3D("sujeto_obligado",  $sujeto_obligado[0]->nombre_sujeto_obligado);

            setD3D("page_act",  "sujetosDetalle");
            $data['page'] = getD3D("page_act");
            $data['campana'] = $string;
            $this->load->view('system/base/bread', $data, FALSE);            
            $data1['ScreenTarget'] = 'Sys_Screen?v='.  getD3D("page_act") . '&g=' . getD3D("group_act") .'&so=' . $string;
