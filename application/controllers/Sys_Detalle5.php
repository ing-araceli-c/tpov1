<?php class Sys_Detalle5 extends CI_Controller {
	...
	return $valor;
 	 ...
 	 if (getD3D("page_act")=='') {
		...
		setD3D("page_act",  "Campanasavisos");
	 }
	 if (isset($_GET['campana'])) {
            if ($_GET['campana']>10000) {
                $string = $_GET['campana']-10000;    
            } else {
                $string = $_GET['campana'];    
            }
            $this->Tpo_model->initialize("tab_campana_aviso");
            $campana = $this->Tpo_model->find1("id_campana_aviso", $string, 1);
            setD3D("campana_name",  $campana[0]->nombre_campana_aviso);
         }
	 ...
	 $this->load->view('system/base/Menu', $data, FALSE);
         ...
         if (isset($_GET['campana'])) {
            ...
            setD3D("page_act",  "campanaDetalle");
            $data['page'] = getD3D("page_act");
            $data['campana'] = $string;
            $this->load->view('system/base/bread', $data, FALSE);            
            $data1['ScreenTarget'] = 'Sys_Screen?v='.  getD3D("page_act") . '&g=' . getD3D("group_act") .'&campana=' . $string;
